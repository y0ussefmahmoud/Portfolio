...
const Services = ({ translations }) => {
    ...
    return (_jsx("section", { id: "services", className: "bg-transparent", children: _jsxs("div", { className: "mx-auto max-w-6xl px-4 py-8", children: [_jsxs("div", { className: "text-center", children: [_jsx("h2", { className: "text-3xl font-bold tracking-tight text-foreground sm:text-4xl", children: translations.services.title }), _jsx("p", { className: "mt-2 text-sm text-muted-foreground sm:text-base", children: translations.services.subtitle })] }), _jsx("div", { className: "mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3", children: servicesData.map((service) => {
                        ...
                        return (_jsx(motion.div, { variants: cardVariants, initial: "hidden", whileInView: "visible", viewport: { once: true, amount: 0.25 }, transition: { duration: 0.5, ease: "easeOut" }, whileHover: {
                                y: -8,
                                scale: 1.02,
                                transition: { type: "spring", stiffness: 300, damping: 20 }
                            }, className: "h-full", children: _jsxs(Card, { className: "group h-full overflow-hidden border-border/60 bg-card/60 backdrop-blur supports-backdrop-filter:bg-card/50 relative", children: [_jsx(motion.div, { className: "absolute inset-0 bg-gradient-to-br from-primary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300", initial: false }), _jsxs(CardHeader, { children: [_jsx(motion.div, { className: "w-16 h-16 mx-auto rounded-full bg-linear-to-br from-primary to-primary/60 flex items-center justify-center", whileHover: {
                                                    scale: 1.1,
                                                    rotate: 5,

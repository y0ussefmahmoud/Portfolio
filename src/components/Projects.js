...
const Projects = ({ translations }) => {
    ...
    const renderProjectCard = (project) => {
        ...
        return (_jsx(motion.div, { variants: cardVariants, initial: "hidden", whileInView: "visible", viewport: { once: true, amount: 0.25 }, transition: { duration: 0.35, ease: 'easeOut' }, whileHover: { y: -4 }, className: "h-full", children: _jsxs(Card, { className: "group h-full overflow-hidden border-border/60 bg-card/60 backdrop-blur supports-backdrop-filter:bg-card/50", children: [_jsxs(CardHeader, { className: "space-y-3", children: [_jsxs("div", { className: "flex items-start justify-between gap-3", children: [_jsx("h3", { className: "text-base font-semibold leading-tight tracking-tight text-foreground", children: project.title }), _jsx(Badge, { variant: project.status === 'completed' ? 'default' : 'secondary', className: "shrink-0", children: project.status === 'completed'
                                    ...
                                    }, className: "h-44 w-full object-cover transition-transform duration-300 ease-out group-hover:scale-[1.03]" }) })] }), _jsxs(CardContent, { className: "space-y-4 flex flex-col", children: [_jsx("p", { className: "text-sm sm:text-base leading-relaxed text-muted-foreground line-clamp-3", children: project.description }), _jsx("div", { className: "flex flex-wrap gap-2", children: project.tech.map((t) => (_jsx(Badge, { variant: "outline", className: "text-xs sm:text-sm", children: t }, t))) })] }), _jsxs(CardFooter, { className: "flex flex-row gap-2", children: [project.details && (_jsxs(Button, { variant: "secondary", onClick: () => setSelectedProject(project), className: "flex-1 text-xs sm:text-sm", shine: true, hoverScale: 1.05, tapScale: 0.95, children: [_jsx(ExternalLink, { className: "h-4 w-4 mr-1 sm:mr-2" }), translations.projects.viewDetails || 'التفاصيل'] })), viewEnabled ? (_jsxs(Button, { variant: "default", onClick: () => window.open(project.viewLink, '_blank'), className: "flex-1 text-xs sm:text-sm", shine: true, hoverScale: 1.05, tapScale: 0.95, children: [_jsx(ExternalLink, { className: "h-4 w-4 mr-1 sm:mr-2" }), translations.projects.view] })) : (_jsxs(Button, { variant: "default", disabled: true, className: "flex-1 text-xs sm:text-sm", children: [_jsx(ExternalLink, { className: "h-4 w-4 mr-1 sm:mr-2" }), translations.projects.view] })), codeEnabled ? (_jsxs(Button, { variant: "outline", onClick: () => window.open(project.codeLink, '_blank'), className: "flex-1 text-xs sm:text-sm", shine: true, hoverScale: 1.05, tapScale: 0.95, children: [_jsx(Github, { className: "h-4 w-4 mr-1 sm:mr-2" }), translations.projects.code] })) : (_jsxs(Button, { variant: "outline", disabled: true, className: "flex-1 text-xs sm:text-sm", children: [_jsx(Github, { className: "h-4 w-4 mr-1 sm:mr-2" }), translations.projects.code] }))] })] }) }, project.id));
    };
